...
def savefile(file, dir):
    """returns saved file path after saving the uploaded file from client,Takes <UploadedFile> and <str>directory to
    save file inside MEDIA ROOT"""
...
def filterupdates(request, instance, fields, dir):
    ...
    for n_field in normal_fields:
        ...
        if str(prev_dict.get(prev)) != request.POST.get(
                n_field):  # request.POST is of type QueryDict and .get() returns <str> of provided key
...
def getInvoiceDetails(pk, context):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            'select core_invoice.invoice_number,core_invoice.customer_name,core_invoice.date_created,core_invoice.total,auth_user.username as created_by from core_invoice inner join auth_user on core_invoice.created_by_id = auth_user.id where invoice_number = %s ',
            [int(pk)])
        invoice_dict = dictfetchall(cursor)[0]
    with connection.cursor() as cursor:
        cursor.execute(
            'select core_invoiceitem.id,core_product.product_name,core_product.unit_price,core_invoiceitem.quantity,core_invoiceitem.accumulated from ((core_invoice inner join core_invoiceitem on core_invoice.invoice_number=core_invoiceitem.invoice_id ) inner join core_product on core_invoiceitem.item_id = core_product.id) where invoice_number = %s ',
            [int(pk)])
